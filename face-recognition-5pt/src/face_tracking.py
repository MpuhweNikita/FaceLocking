...
class ActionLogger:
    def __init__(self, log_file="action_history.txt"):
    ...
    def log(self, action_type, description):
        ...
        entry = f"{timestamp} | {action_type} | {description}"
...
def main():
    ...
    action_logger = ActionLogger("action_history.txt")
    action_logger.log("SYSTEM", "Application Started")
    prev_state = None
    prev_h = "CENTER"
    prev_v = "CENTER"
    ...
    try:
        while True:
            ...
            if tracker.state != prev_state:
                if tracker.state == LockState.LOCKED:
                    action_logger.log("LOCK_ACQUIRED", f"Locked onto identity: {args.target}")
                elif tracker.state == LockState.LOST:
                    action_logger.log("TRACKING_LOST", f"Lost sight of {args.target}, waiting for return...")
                elif tracker.state == LockState.SEARCHING:
                    if prev_state == LockState.LOST:
                        action_logger.log("LOCK_RELEASED", f"Sustained disappearance of {args.target}, lock released.")
                    else:
                        print(f"[SEARCHING] Looking for target {args.target}...")
                prev_state = tracker.state
            
            if tracker.state == LockState.LOCKED and position:
                if position.horizontal != prev_h and position.horizontal != "CENTER":
                    action_logger.log("FACE_MOVEMENT", f"Locked face moved {position.horizontal.lower()}")
                if position.vertical != prev_v and position.vertical != "CENTER":
                    action_logger.log("FACE_MOVEMENT", f"Locked face moved {position.vertical.lower()}")
                prev_h = position.horizontal
                prev_v = position.vertical
            ...
            if args.signal:
                ...
                # Draw recent actions on screen
                y_offset = view.shape[0] - 100
                draw_label(view, "Recent Actions:", (12, y_offset), (255, 255, 255), 0.5)
                for i, action in enumerate(action_logger.get_recent(4)):
                    draw_label(view, action, (12, y_offset + 20 + i*20), (200, 200, 200), 0.45)
